...
exports.save_product = async (req, res) => {
  const { p_name, p_size, p_qty, p_desc, p_price, p_d_price, files, m_title, m_keywords, m_description } = req.body;
  const requiredFields = ['p_name', 'p_size', 'p_qty', 'p_desc', 'p_price'];
  const emptyFields = requiredFields.filter(field => !req.body[field]);
  const sku = 'sku_' + randomstring.generate({
    length: 6,
...
exports.update_product = async (req, res) => {
  const { p_name, p_size, p_qty, p_desc, p_price, p_d_price, m_title, m_keywords, m_description } = req.body;
  const requiredFields = ['p_name', 'p_size', 'p_qty', 'p_desc', 'p_price'];
  const emptyFields = requiredFields.filter(field => !req.body[field]);
